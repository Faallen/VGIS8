...
def new_kalman_filter(kalman_filters_list, predictions_list, person, blob_id):
    ...
    # Index 15 - weight for calculations of running
    r = randint(0, 255)
    g = randint(0, 255)
    b = randint(0, 255)
...
input: velocity data over 10 frames, min running velocity
output: boolean
'''
def suspicious_running(vel_data, t_vel_running, t_acc_running):
    
    aver_vel = np.zeros((8))
    aver_a = np.zeros((7))

    for i in range(8):
        aver_vel[i] = vel_data[i][1]
        
    for i in range(7):
        aver_a[i] = (vel_data[i+1][1]-vel_data[i][1])/(0.3)

    aver_vel = sorted(aver_vel)
    aver_a = sorted(aver_a)
    average_vel = aver_vel[4]
    average_acc = aver_a[3]
...
'''
Detect running
input: predictions_list, vel_data, frame number
'''
def detect_running(predictions_list, vel_data, nr_frame):
    ...
    for detection in predictions_list:
        ...
        if detection[1] == 0:
            ...
            if detection[10] in vel_data:
                if len(vel_data[detection[10]]) > 7:
            ...
            if len(vel_data[detection[10]]) > 7:
                ...
                t_vel_running = detection[3][-1][1]/1.2
                t_acc_running = detection[3][-1][1]/3.3
                
                if suspicious_running(vel_data[detection[10]], t_vel_running, 
                                      t_acc_running):
                    
                    if detection[6] == "Person":
                        detection[15] += 1
                        if detection[15] > 3 and detection[14] == "" or detection[14]=="Running":
                            detection[8] = True
                            detection[14] = "Running"
                else:
                    if detection[6] == "Person":
                        detection[15] -= 0.2
                    else:
                    ...
                    if detection[15] < 0:
                        detection[15] = 0
                    if detection[15] == 0 and detection[14] == "Running":
                        detection[14] = ""
                        detection[8] = False
